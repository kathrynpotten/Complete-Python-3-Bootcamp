...
class Player:
    ...
    def hand(self):
        return ', '.join(str(card) for card in player.all_cards)
    ...
    def initial_money_pot(self):
        while True:
            try:
                player.add_money(int(input('How much money do you have for the game? ')))
            except TypeError:
    ...
    def choose_aces(self):
        player_choice = input("Would you like to choose the value with aces eleven? (yes/no) ")
